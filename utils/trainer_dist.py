...
class ModelTrainer:
    ...
    def __init__(self,
        ...
        param:
            ...
            chkp_path: path to the checkpoint that needs to be loaded (None for
                       new training)
            finetune: finetune from checkpoint (True) or restore training from
                      checkpoint (False)
    ...
    def train(self, net, training_loader, val_loader, config):
        ...
        # Start training loop
        for epoch in range(config.max_epoch + 2):
            ...
            for batch in training_loader:
                ...
                # step6. deal with the distributed loss with all_reduce
    ...
    def cloud_segmentation_validation(self,
        ...
        if not hasattr(self, 'validation_probs'):
            self.validation_probs = [
                np.zeros((label.shape[0], nc_model))
                for label in val_loader.dataset.input_labels
            ]
