                ...
                if int(np.ceil(new_min)) % 10 == 0:
                    ...
                    if test_loader.dataset.set == 'validation':
                        ...
                        for i, file_path in enumerate(
                            ...
                            print('proj_probs[i]', proj_probs[i].shape)
                            # Insert false columns for ignored labels
                            for l_ind, label_value in enumerate(
                                    test_loader.dataset.label_values):
                                if label_value in test_loader.dataset.ignored_labels:
                            ...
                            print('label_values',
                                  test_loader.dataset.label_values, 'targets',
                                  targets.dtype, type(targets), targets.shape,
                                  np.unique(targets), 'preds', preds.shape,
                                  np.unique(preds), 'proj_probs[i]',
                                  proj_probs[i].shape)
                            Confs += [
                                fast_confusion(
                                    targets, preds,
