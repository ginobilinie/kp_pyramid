...
class ModelTrainer:
    ...
    def __init__(self,
                 ...
                 finetune=False,
                 device=None):
        """Initialize training parameters and reload previous model for
        restore/finetune.

        param:
            net: network object
            config: configuration object
            chkp_path: path to the checkpoint that needs to be loaded (None for new training)
            finetune: finetune from checkpoint (True) or restore training from checkpoint (False)
            on_gpu: Train on GPU or CPU
        """
        ...
        other_params = [
            v for k, v in net.named_parameters() if 'offset' not in k
        ]
    ...
    def train(self, net, training_loader, val_loader, config):
        ...
        for epoch in range(config.max_epoch):
            ...
            for batch in training_loader:
                ...
                # Forward pass
                _, loss, acc = net(batch, config)

                # step6. deal with the distributed loss with all_reduce, # dong nie
                dist.all_reduce(loss, dist.ReduceOp.SUM)
                loss = loss / config.world_size
                ...
                if (t[-1] - last_display) > 1.0:
                    last_display = t[-1]
                    message = (
                        'e{:03d}-i{:04d} => Loss={:.3f} LR={:.6f} '
                        'acc={:3.0f}% / t(ms): {:5.1f} {:5.1f} {:5.1f})')
                    print(
                        message.format(self.epoch, self.step, loss.item(),
                                       self.curr_lr, 100 * acc,
                                       1000 * mean_dt[0], 1000 * mean_dt[1],
                                       1000 * mean_dt[2]))

                # Log file
                if config.saving:
                    with open(join(config.saving_path, 'training.txt'),
                              'a') as file:
                        message = '{:d} {:d} {:.3f} {:.3f} {:.3f}\n'
                        file.write(
                            message.format(self.epoch, self.step, loss, acc,
                                           t[-1] - t0))
            ...
            if self.epoch in config.lr_decays:
                self.curr_lr *= config.lr_decays[self.epoch]
            ...
            # Saving
            if config.saving and torch.distributed.get_rank() == 0:
                ...
                checkpoint_path = join(checkpoint_directory,
                                       'current_chkp.tar')
                ...
                if (self.epoch) % config.checkpoint_gap == 0:
                    checkpoint_path = join(
                        ...
                        'chkp_{:04d}.tar'.format(self.epoch))
                    torch.save(save_dict, checkpoint_path)

                del save_dict
                del new_state_dict

            # Validation, since validation code is not ready for xmap3d, we donot validate now
            if (self.epoch) % 10 == 0:
                net.eval()
                with torch.no_grad():
    ...
    def cloud_segmentation_validation(self,
                                      net,
                                      val_loader,
                                      config,
                                      debug=False):
        ...
        if not hasattr(self, 'validation_probs'):
            self.validation_probs = [
                np.zeros((num, nc_model))
                for num in val_loader.dataset.input_points
            ]
        ...
        if config.saving:
            ...
            for i, file_path in enumerate(files):
                KDTree_file = val_loader.dataset.input_trees[i]
                with open(KDTree_file, 'rb') as f:
                    input_tree = pickle.load(f)

                pot_points = np.array(input_tree.data, copy=False)
        ...
        for IoU in IoUs:
            line += '{:.2f} '.format(IoU * 100)

        print('{:s} mean IoU = {:.1f}%|{:s}'.format(config.dataset, mIoU,
                                                    line))

        # Save predicted cloud occasionally
        if config.saving and (
                self.epoch) % config.checkpoint_gap == 0 and 1 == 0:
            val_path = join(config.saving_path,
                            'val_preds_{:d}'.format(self.epoch))
