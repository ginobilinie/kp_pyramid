...
def fast_confusion(true, pred, label_values=None):
    """Fast confusion matrix (100x faster than Scikit learn). But only works if
    labels are la.
    ...
    if len(true.shape) != 1:
        raise ValueError(
            'Truth values are stored in a {:d}D array instead of 1D array'.
            format(len(true.shape)))
    if len(pred.shape) != 1:
        raise ValueError(
            'Prediction values are stored in a {:d}D array instead of 1D array'
            .format(len(pred.shape)))
    if true.dtype not in [np.int32, np.int64]:
        raise ValueError(
            'Truth values are {:s} instead of int32 or int64'.format(
                true.dtype))
    if pred.dtype not in [np.int32, np.int64]:
        raise ValueError(
            'Prediction values are {:s} instead of int32 or int64'.format(
                pred.dtype))
    ...
    if label_values[0] == 0 and label_values[-1] == num_classes - 1:
        ...
        if vec_conf.shape[0] < num_classes**2:
            vec_conf = np.pad(vec_conf,
                              (0, num_classes**2 - vec_conf.shape[0]),
                              'constant')
...
def metrics(confusions, ignore_unclassified=False):
    ...
    param :
        confusions: ([..., n_c, n_c] np.int32). Can be any dimension, the confusion
                    matrices should be described by the last axes. n_c = number of classes
        ignore_unclassified: (bool). True if the the first class should be ignored
                             in the results
    return:
        ([..., n_c] np.float32) precision, recall, F1 score, IoU score
    """
    ...
    # Compute TP, FP, FN. This assume that the second to last axis counts the
    # truths (like the first axis of a confusion matrix), and that the last axis
    # counts the predictions (like the second axis of a confusion matrix)
    TP = np.diagonal(confusions, axis1=-2, axis2=-1)
    TP_plus_FP = np.sum(confusions, axis=-1)
    TP_plus_FN = np.sum(confusions, axis=-2)

    # Compute precision and recall. This assume that the second to last axis counts
    # the truths (like the first axis of a confusion matrix), and that the last axis
    # counts the predictions (like the second axis of a confusion matrix)
...
def smooth_metrics(confusions, smooth_n=0, ignore_unclassified=False):
    """Computes different metrics from confusion matrices. Smoothed over a
    number of epochs.

    param:
        confusions: ([..., n_c, n_c] np.int32). Can be any dimension, the confusion
                    matrices should be described by the last axes. n_c = number of classes
        smooth_n: (int). smooth extent
        ignore_unclassified: (bool). True if the the first class should be ignored
                             in the results
    return:
        ([..., n_c] np.float32) precision, recall, F1 score, IoU score
    ...
    # Compute TP, FP, FN. This assume that the second to last axis counts the truths
    # (like the first axis of a onfusion matrix), and that the last axis counts the
    # predictions (like the second axis of a confusion matrix)
    ...
    # Compute Accuracy
    ACC = np.sum(TP,
                 axis=-1) / (np.sum(smoothed_confusions, axis=(-2, -1)) + 1e-6)
...
def IoU_from_confusions(confusions):
    ...
    # Compute TP, FP, FN. This assume that the second to last axis counts the truths
    # (like the first axis of a confusion matrix), and that the last axis counts the
    # predictions (like the second axis of a confusion matrix)
    TP = np.diagonal(confusions, axis1=-2, axis2=-1)
