...
from arch_config import get_architectures
...
class Config:
    ...
    def __init__(self, arch_name=None):
        ...
        self.architecture = get_architectures(arch_name)
        # Number of layers
        if len(self.architecture) == 0 or type(
                self.architecture[0]) is not list:
            self.num_layers = len([
            ...
            self.num_layers = len([
                block for block in self.architecture[0]
                if 'pool' in block or 'strided' in block
            ]) + 1
        ...
        if len(layer_blocks) > 0:
            if np.any(['deformable' in blck for blck in layer_blocks]):
                self.deform_layers += [True]
            else:
                self.deform_layers += [False]
