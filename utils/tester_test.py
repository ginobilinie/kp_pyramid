...
class ModelTester:
    ...
    def cloud_segmentation_test(self,
        ...
        if test_loader.dataset.set == 'validation':
            if config.dataset.lower() == 'xmap3d' or config.dataset.lower(
            ) == 'xmap3d_kp':
                self.test_probs = [
                    np.zeros((num, nc_model))
                    for num in test_loader.dataset.input_points
                ]
            else:
                self.test_probs = [
                    np.zeros((label.shape[0], nc_model))
                    for label in test_loader.dataset.input_labels
                ]
        else:
            if config.dataset.lower() == 'xmap3d' or config.dataset.lower(
            ) == 'xmap3d_kp':
                ...
                self.test_probs = [
                    np.zeros((label.shape[0], nc_model))
                    for label in test_loader.dataset.input_colors
                ]
        ...
        while True:
            ...
            if last_min + 1 < new_min:
                ...
                if config.dataset.lower() == 'xmap3d' or config.dataset.lower(
                    ...
                    full_cloud_interval = 1
                else:
                    full_cloud_interval = 10
                if int(np.ceil(new_min)) % full_cloud_interval == 0:
                    ...
                    if test_loader.dataset.set == 'validation':
                        ...
                        for l_ind, label_value in reversed(
                            ...
                            if label_value in test_loader.dataset.ignored_labels:
                                C = np.delete(C, l_ind, axis=0)
                                C = np.delete(C, l_ind, axis=1)
                        print('confusion matrix on full cloud:')
                        print(C)
                        IoUs = IoU_from_confusions(C)
                        mIoU = np.mean(IoUs)
                        s = '{:5.2f} | '.format(100 * mIoU)
                    ...
                    for i, file_path in enumerate(test_loader.dataset.files):
                        ...
                        # Save potentials
                        if len(test_loader.dataset.pot_trees) > 0:
                            pot_points = np.array(
                                test_loader.dataset.pot_trees[i].data,
                                copy=False)
                            pot_name = join(test_path, 'potentials',
                                            cloud_name)
        ...
        while True:
            ...
            for i, batch in enumerate(test_loader):
                ...
                for b_i, length in enumerate(lengths):
                    ...
                    if test_loader.dataset.set == 'validation':
                        ...
                        if f_ind % 20 == 0:
                            ...
                            lbl_names = [
                                test_loader.dataset.label_to_names[label]
                                for label in test_loader.dataset.label_values
                                if label not in
                ...
                if (t[-1] - last_display) > 1.0:
                    last_display = t[-1]
                    message = (
                        'e{:03d}-i{:04d} => {:.1f}% (timings : '
                        '{:4.2f} {:4.2f} {:4.2f}) / pots {:d} => {:.1f}%')
                    min_pot = int(
