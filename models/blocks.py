...
def closest_pool(x, inds):
    """Pools features from the closest neighbors. WARNING: this function
    assumes the neighbors are ordered.
...
class KPConv(nn.Module):
    def __init__(self,
        ...
        param:
            ...
            radius: radius used for kernel point init. Even for deformable, use the
                    config.conv_radius
            fixed_kernel_points: fix position of certain kernel points ('none',
                                 'center' or 'verticals').
            KP_influence: influence function of the kernel points ('constant',
                          'linear', 'gaussian').
            aggregation_mode: choose to sum influences, or only keep the closest
                              ('closest', 'sum').
        ...
        # Running variable containing deformed KP distance to input points.
        # (used in regularization loss)
        self.min_d2 = None
    ...
    def forward(self, q_pts, s_pts, neighb_inds, x):
        ...
        if self.deformable:
            ...
            # For each row of neighbors, indices of the ones that are in range
            # [n_points, new_max_neighb]
        ...
        if self.KP_influence == 'constant':
            ...
            # Influence decrease linearly with the distance, and get to zero
            # when d = KP_extent.
...
def block_decider(block_name, radius, in_dim, out_dim, layer_ind, config):
    ...
    if block_name == 'unary':
        ...
        if hasattr(config, 'resblock') and config.resblock == 'heavy':
            return ResnetBottleneckBlockTF(block_name, in_dim, out_dim, radius,
                                           layer_ind, config)
        else:
...
class BatchNormBlock(nn.Module):
    def __init__(self, in_dim, use_bn, bn_momentum):
        """Initialize a batch normalization block. If network does not use
        batch normalization, replace with biases.
...
class ResnetBottleneckBlockTF(nn.Module):
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
        """This is a version which exactly matches the one in tensorflow
        version.
        ...
        :param config: parameters
        """
        super(ResnetBottleneckBlockTF, self).__init__()
        ...
        # First downscaling mlp
        if in_dim != out_dim // 2:
            self.unary1 = UnaryBlock(in_dim, out_dim // 2, self.use_bn,
                                     self.bn_momentum)
        else:
            self.unary1 = nn.Identity()
        ...
        self.KPConv = KPConv(config.num_kernel_points,
                             ...
                             deformable='deform' in block_name,
                             modulated=config.modulated)
        self.batch_norm_conv = BatchNormBlock(out_dim // 2, self.use_bn,
                                              self.bn_momentum)

        # Second upscaling mlp
        self.unary2 = UnaryBlock(out_dim // 2,
                                 out_dim,
                                 self.use_bn,
                                 self.bn_momentum,
                                 no_relu=True)
...
class NearestUpsampleBlock(nn.Module):
    def __init__(self, layer_ind):
        """Initialize a nearest upsampling block with its ReLU and
        BatchNorm."""
