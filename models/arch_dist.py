...
from models.myloss import (Combine_CE_Dice_Loss, Combine_Dice_Focal_Loss,
                           Combine_Ohem_Dice_Loss, FocalLoss)
...
class KPFCNN(nn.Module):
    ...
    def __init__(self, config, lbl_values, ign_lbls):
        ...
        if config.loss_type == 'focal_loss':
            self.criterion = FocalLoss(weight=class_w,
                                       reduction='sum',
                                       ignore_indx=-1)
        if config.loss_type == 'Dice_Ohem':
            self.criterion = Combine_Ohem_Dice_Loss(
                mylambda_dice=1,
                mylambda_ohem=0.7,
                ignore_label=-1,
                thresh=0.7,
                min_kept=int(config.batch_size * config.num_points // 16))
        elif config.loss_type == 'Dice_CE':
            self.criterion = Combine_CE_Dice_Loss(mylambda=0.7,
                                                  ignore_label=-1)
        elif config.loss_type == 'Dice_Focal':
            self.criterion = Combine_Dice_Focal_Loss(mylambda_dice=0.7,
                                                     mylambda_focal=1.3,
                                                     ignore_label=-1,
