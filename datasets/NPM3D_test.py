...
class NPM3DDataset(PointCloudDataset):
    ...
    def __init__(self,
                 ...
                 subdata_path=None,
                 on_val=False):
        """This dataset is small enough to be stored in-memory, so load all
        point clouds here."""
        PointCloudDataset.__init__(self, 'NPM3D', config)
        ...
        self.path = 'data'
        if subdata_path is not None:
            self.path = subdata_path
        print('data_path:', self.data_path)
        print('subdata_path:', self.path)
        ...
        self.train_path = 'training_points'
        if self.set != 'test':
            # List of files to process
            ply_path = join(self.data_path, self.train_path)

            # Proportion of validation scenes
            self.cloud_names = ['Lille1_1', 'Lille1_2', 'Lille2', 'Paris']
        ...
        # self.prepare_NPM3D_ply() # no necessary for NPM3D
        ...
        if self.set == 'training':
            self.cloud_names = [
                f for i, f in enumerate(self.cloud_names)
                if self.all_splits[i] != self.validation_split
            ]
        elif self.set in ['validation', 'ERF']:
            self.cloud_names = [
        ...
        if use_potentials:
            ...
            N = self.epoch_n
            self.epoch_inds = torch.from_numpy(np.zeros((2, N),
                                                        dtype=np.int64))
        ...
        # For ERF visualization, we want only one cloud per batch and
        # no randomness
        if self.set == 'ERF':
            self.batch_limit = torch.tensor([1], dtype=torch.float32)
            self.batch_limit.share_memory_()
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        while True:
            ...
            with self.worker_lock:
                ...
                if self.set != 'ERF':
                    tukeys = np.square(1 -
                                       d2s / np.square(self.config.in_radius))
            ...
            input_inds = self.input_trees[cloud_ind].query_radius(
                center_point, r=self.config.in_radius)[0]
            ...
            if self.set == 'training':
                # Randomly drop some points (act as an augmentation
                # process and a safety for GPU memory consumption)
                if n > int(self.batch_limit - batch_n):
                    """mem_debug."""
                    if mem_debug:
                        np.random.seed(0)

                    sample_ids = np.random.choice(n,
                                                  size=int(self.batch_limit -
                                                           batch_n),
                                                  replace=False)

                    input_points = input_points[sample_ids]
                    input_features = input_features[sample_ids]
                    input_labels = input_labels[sample_ids]
                    input_inds = input_inds[sample_ids]
                    n = input_inds.shape[0]
            ...
            if batch_n >= int(self.batch_limit):
                if self.set == 'training':
                    assert (batch_n == int(self.batch_limit))
                break
        ...
        if mem_debug:
            print('dbg stacked_length:', stack_lengths)
        ...
        if self.config.in_features_dim == 1:
            ...
            raise ValueError('Only accepted input dimensions are 1, 4 and 7'
    ...
    def random_item(self, batch_i):
        ...
        while True:
            ...
            # Add a small noise to center point
            if self.set != 'ERF':
                center_point += np.random.normal(scale=self.config.in_radius /
                                                 10,
                                                 size=center_point.shape)
    ...
    def prepare_NPM3D_ply(self):
        ...
        raise ValueError('prepare_NPM3D_ply is not implemented')
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                reflectance = np.expand_dims(data['reflectance'], 1)
                if self.set != 'test':
                    labels = data['class']

                    # Subsample cloud
                    sub_points, sub_reflectance, sub_labels = grid_subsampling(
                        points,
                    ...
                    # Subsample cloud
                    sub_points, sub_reflectance = grid_subsampling(
                        points, features=reflectance, sampleDl=dl)
                ...
                if self.set != 'test':
                    ...
                    write_ply(sub_ply_file, [sub_points, sub_reflectance],
                              ['x', 'y', 'z', 'reflectance'])
            ...
            size = sub_reflectance.shape[0] * 1 * 5
            ...
            for i, file_path in enumerate(self.files):
                ...
                if exists(coarse_KDTree_file):
                    ...
                    sub_points = np.array(self.input_trees[cloud_ind].data,
                                          copy=False)
                    coarse_points = grid_subsampling(sub_points.astype(
                        np.float32),
                                                     sampleDl=pot_dl)
    ...
    def load_evaluation_points(self, file_path):
        """Load points (from test or validation split) on which the metrics
        should be evaluated."""
...
class NPM3DSampler(Sampler):
    ...
    def __iter__(self):
        ...
        In this dataset, this is a dummy sampler that yield the index of batch element (input
        sphere) in epoch instead of the list of point indices
        """
        ...
        if not self.dataset.use_potentials:
            ...
            random_pick_n = int(
                np.ceil(num_centers / (self.dataset.num_clouds *
                                       self.dataset.config.num_classes)))
            ...
            if self.set != 'test':
                for cloud_ind, cloud_labels in enumerate(
                    ...
                    for label_ind, label in enumerate(
                        ...
                        if label not in self.dataset.ignored_labels:
                            ...
                            if len(label_indices) <= random_pick_n:
                                ...
                                epoch_indices = np.hstack(
                                    (epoch_indices,
                                     rand_inds[:random_pick_n].astype(
                                         np.int32)))

                    # Stack those indices with the cloud index
                    epoch_indices = np.vstack(
                        (np.full(epoch_indices.shape,
                                 cloud_ind,
                                 dtype=np.int32), epoch_indices))

                    # Update the global indice container
                    all_epoch_inds = np.hstack((all_epoch_inds, epoch_indices))
            else:
                for cloud_ind, cloud_colors in enumerate(
                        self.dataset.input_colors):
                    epoch_indices = np.empty((0, ), dtype=np.int32)
                    label_indices = np.arange(cloud_colors.shape[0])
                    if len(
                            label_indices
                    ) <= random_pick_n * self.dataset.config.num_classes:
                        epoch_indices = np.hstack(
                            (epoch_indices, label_indices))
                    else:
                        new_randoms = np.random.choice(
            ...
            # Update epoch inds
            # myind = all_epoch_inds[:, :num_centers]
    ...
    def fast_calib(self):
        """This method calibrates the batch sizes while ensuring the potentials
        are well initialized.

        Indeed on a dataset
        like Semantic3D, before potential have been updated over the dataset,
        there are cahnces that all the dense area are picked in the begining
        and in the end, we will have very large batch of small point clouds
        return:
        """
        ...
        for epoch in range(10):
            for i, test in enumerate(self):
                ...
                if (t[-1] - last_display) > 1.0:
                    last_display = t[-1]
                    message = 'Step {:5d}  estim_b ={:5.2f} batch_limit ={:7d},  //  {:.1f}ms {:.1f}ms'  # noqa
                    print(
    ...
    def calibration(self,
        ...
        Batch calibration: Set "batch_limit" (the maximum number of points
        allowed in every batch) so that the                        average
        batch size (number of stacked pointclouds) is the one asked. Neighbors
        calibration: Set the "neighborhood_limits" (the maximum number of
        neighbors allowed in convolutions)                        so that 90%
        of the neighborhoods remain untouched. There is a limit for each layer.
        """
        ...
        key = '{:s}_{:.3f}_{:.3f}_{:d}'.format(
            sampler_method, self.dataset.config.in_radius,
...
class NPM3DCustomBatch:
    ...
    '''
        to move to cuda for multiple gpus
        dong nie
    '''

    def cuda(self):

        self.points = [in_tensor.cuda() for in_tensor in self.points]
        self.neighbors = [in_tensor.cuda() for in_tensor in self.neighbors]
        self.pools = [in_tensor.cuda() for in_tensor in self.pools]
        self.upsamples = [in_tensor.cuda() for in_tensor in self.upsamples]
        self.lengths = [in_tensor.cuda() for in_tensor in self.lengths]
        self.features = self.features.cuda()
        self.labels = self.labels.cuda()
        self.scales = self.scales.cuda()
        self.rots = self.rots.cuda()
        self.cloud_inds = self.cloud_inds.cuda()
        self.center_inds = self.center_inds.cuda()
        self.input_inds = self.input_inds.cuda()
    ...
    for epoch in range(10):
        ...
        for batch_i, batch in enumerate(loader):
            ...
            if (t[-1] - last_display) > -1.0:
                ...
                print(
                    message.format(batch_i, 1000 * mean_dt[0],
                                   1000 * mean_dt[1], estim_b, estim_N))
