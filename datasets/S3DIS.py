...
class S3DISDataset(PointCloudDataset):
    ...
    def __init__(self,
        ...
        point clouds here."""
        PointCloudDataset.__init__(self, 'S3DIS', config)
    ...
    def prepare_S3DIS_ply(self):
        ...
        for cloud_name in self.cloud_names:
            ...
            room_folders = [
                join(cloud_folder, room) for room in listdir(cloud_folder)
                ...
                for object_name in listdir(join(room_folder, 'Annotations')):
                    ...
                    if object_name[-4:] == '.txt':
                        ...
                        cloud_points = np.vstack(
                            (cloud_points,
                             object_data[:, 0:3].astype(np.float32)))
                        cloud_colors = np.vstack(
                            (cloud_colors, object_data[:,
                                                       3:6].astype(np.uint8)))
            ...
            # Save as ply
            write_ply(cloud_file, (cloud_points, cloud_colors, cloud_classes),
