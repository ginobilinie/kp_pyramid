...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Class handling Semantic3D dataset in pytorch version.
...
class Semantic3DDataset(PointCloudDataset):
    ...
    def __init__(self,
        ...
        point clouds here."""
        PointCloudDataset.__init__(self, 'Semantic3D', config)
        ...
        self.sub_pc_folder = join(
            self.path, 'input_{:.3f}'.format(config.first_subsampling_dl))
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        stacked_features = np.ones_like(stacked_points[:, :1],
                                        dtype=np.float32)
        if self.config.in_features_dim == 1:  # 1,
            pass
        elif self.config.in_features_dim == 4:  # 1, r,g,b,
            stacked_features = np.hstack((stacked_features, features[:, :3]))
        elif self.config.in_features_dim == 5:  # 1, r,g,b, z
    ...
    def random_item(self, batch_i):
        ...
        if self.config.in_features_dim == 1:  # 1,
            ...
            stacked_features = np.hstack(
                (stacked_features, features[:, :3]))  # 1, r,g,b,
        elif self.config.in_features_dim == 5:
            stacked_features = np.hstack(
                (stacked_features, features))  # 1, r,g,b, z
        else:
            raise ValueError(
                'Only accepted input dimensions are 1, 4 and 7 (without and with XYZ)'
            )
    ...
    def prepare_data(self):
        ...
        cloud_names = [
            file_name[:-4] for file_name in listdir(old_folder)
            if file_name[-4:] == '.txt'
        ]
        ...
        for cloud_name in cloud_names:
            ...
            if exists(label_file):
                ...
                sub_points, sub_colors, sub_labels = grid_subsampling(
                    points, features=colors, labels=labels, sampleDl=0.01)

                # Write the subsampled ply file
                write_ply(ply_file_full, (sub_points, sub_colors, sub_labels),
                          ['x', 'y', 'z', 'red', 'green', 'blue', 'class'])

            else:

                # Write the full ply file
                write_ply(ply_file_full, (points, colors),
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                if self.set != 'test':
                    ...
                    write_ply(sub_ply_file, [sub_points, sub_colors],
                              ['x', 'y', 'z', 'red', 'green', 'blue'])
...
class Semantic3DSampler(Sampler):
    ...
    def __init__(self, dataset: Semantic3DDataset):
        ...
        if dataset.set == 'training':
            self.N = dataset.config.epoch_steps  # steps for each epoch
        else:
        ...
        if not self.dataset.use_potentials:
            ...
            random_pick_n = int(
                np.ceil(num_centers / (self.dataset.num_clouds *
                                       (self.dataset.config.num_classes - 1))))
        ...
        for epoch in range(10):
            for i, test in enumerate(self):
                ...
                if (t[-1] - last_display) > 1.0:
                    last_display = t[-1]
                    message = ('Step {:5d}  estim_b ={:5.2f} '
                               'batch_limit ={:7d},  //  {:.1f}ms {:.1f}ms')
                    print(
