...
class NPM3DDataset(PointCloudDataset):
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        while True:
            ...
            if self.set == 'training':
                # Randomly drop some points (act as an augmentation process and
                # a safety for GPU memory consumption)
    ...
    def random_item(self, batch_i):
        ...
        while True:
            ...
            # Color augmentation -- linear transform
            if np.random.rand() > 0.5 and self.set == 'training':
                rand_shift = np.random.randn()
                rand_scale = 0.9 + 0.2 * np.random.rand()
                if np.abs(rand_shift) <= 2.58:
                    # (300/6000) / 1.96
                    input_colors = input_colors * rand_scale + rand_shift * 0.025510204081632654
            ...
            # Randomly drop some points (act as an augmentation process and a
            # safety for GPU memory consumption)
            # if n > int(self.batch_limit):
            #    input_inds = np.random.choice(input_inds,
            #                   size=int(self.batch_limit) - 1, replace=False)
            #    n = input_inds.shape[0]
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                sub_points, sub_reflectance, sub_labels = grid_subsampling(
                    points, features=reflectance, labels=labels, sampleDl=dl)
...
class NPM3DSampler(Sampler):
            ...
            for cloud_ind, cloud_labels in enumerate(
                ...
                for label_ind, label in enumerate(self.dataset.label_values):
                    if label not in self.dataset.ignored_labels:
                        label_indices = np.where(np.equal(cloud_labels,
                                                          label))[0]
                        if len(label_indices) <= random_pick_n:
                            ...
                            epoch_indices = np.hstack(
                                (epoch_indices,
                                 rand_inds[:random_pick_n].astype(np.int32)))

                # Stack those indices with the cloud index
                epoch_indices = np.vstack(
                    (np.full(epoch_indices.shape, cloud_ind,
    ...
    def fast_calib(self):
        ...
        Indeed on a dataset like Semantic3D, before potential have been updated over the dataset,
        there are cahnces that all the dense area are picked in the begining and in the end, we will
        have very large batch of small point clouds
        """
        ...
        for epoch in range(10):
            for i, test in enumerate(self):
                ...
                if (t[-1] - last_display) > 1.0:
                    last_display = t[-1]
                    message = (
                        'Step {:5d}  estim_b ={:5.2f} batch_limit ={:7d},'
                        '  //  {:.1f}ms {:.1f}ms')
                    print(
    ...
    def calibration(self,
        ...
        so that the average batch size (number of stacked pointclouds) is the one asked. Neighbors
        calibration: Set the "neighborhood_limits" (the maximum number of neighbors allowed in
        convolutions). so that 90% of the neighborhoods remain untouched. There is a limit for each
        layer.
        """
