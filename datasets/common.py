...
class PointCloudDataset(Dataset):
    ...
    def augmentation_transform(self, points, normals=None, verbose=False):
        ...
        noise = (np.random.randn(points.shape[0], points.shape[1]) *
                 self.config.augment_noise).astype(np.float32)
    ...
    def classification_inputs(self, stacked_points, stacked_features, labels,
                              stack_lengths):
        ...
        for block_i, block in enumerate(arch):
            ...
            deform_layer = False
            if layer_blocks:
                # Convolutions are done in this layer, compute the neighbors
                # with the good radius
    ...
    def segmentation_inputs(self, stacked_points, stacked_features, labels,
        ...
        # Loop over the blocks
        ######################
        if len(self.config.architecture) == 0 or type(
                self.config.architecture[0]) is not list:
            arch = self.config.architecture
        else:
            arch = self.config.architecture[0]
